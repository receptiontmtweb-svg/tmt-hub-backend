...
// =====================================================================
// RESTRICTED DATA TOKEN (RDT) — requis pour accéder aux données PII
// (BuyerName, BuyerEmail, ShippingAddress.Name, Phone, etc.)
// Doc : POST /tokens/2021-03-01/restrictedDataTokens
// =====================================================================
async function getRestrictedDataToken(lwaToken, orderId, dataElements = []) {
  // dataElements vides = accès à tous les champs restreints de la ressource
  const targetedResources = [];

  if (dataElements.includes('buyerInfo') || dataElements.length === 0) {
    targetedResources.push({
      path:         `/orders/v0/orders/${orderId}/buyerInfo`,
      dataElements: ['buyerInfo'],
    });
  }

  if (dataElements.includes('shippingAddress') || dataElements.length === 0) {
    targetedResources.push({
  ...
  try {
    const res = await axios.post(
      `${SP_API_BASE}/tokens/2021-03-01/restrictedDataTokens`,
      { restrictedResources: targetedResources },
      {
        headers: {
    ...
    const rdt = res?.data?.restrictedDataToken;

    if (!rdt) {
      throw new Error('RDT manquant dans la réponse Amazon Tokens API');
    }

    Logger.info('amazon-connector', 'AMZ_RDT_OK', {
      orderId,
      resources: targetedResources.map(r => r.path),
    ...
    Logger.error('amazon-connector', 'AMZ_RDT_FAIL', {
      orderId,
      status,
      err: _axErr(e),
      note: status === 403
        ? 'App SP-API sans role PII ou dataElements non autorisés'
        : 'Erreur inattendue RDT',
    });
    // On retourne null : les fonctions appelantes continueront avec le token LWA
    // (Amazon retournera les champs non-PII, PII masqués)
    return null;
  }
}
...
// Utilise un Restricted Data Token (RDT) pour accéder aux données PII
// =====================================================================
async function getOrderBuyerInfo(lwaToken, orderId, retryCount = 0) {
  try {
    await sleep(400);

    // ── Obtenir un RDT pour accéder aux PII BuyerName/BuyerEmail ────
    const rdt = await getRestrictedDataToken(lwaToken, orderId, ['buyerInfo']);
    // Si RDT indisponible, on tente quand même avec le LWA token
    // (résultat probable : champs PII masqués, mais non bloquant)
    const activeToken = rdt || lwaToken;

    if (!rdt) {
      Logger.warn('amazon-connector', 'AMZ_BUYER_INFO_NO_RDT', {
        orderId,
        note: 'RDT non disponible, appel avec LWA token — PII probablement masqués',
      });
    }

    const res = await axios.get(
      `${SP_API_BASE}/orders/v0/orders/${encodeURIComponent(orderId)}/buyerInfo`,
      { headers: { 'x-amz-access-token': activeToken } }
    );
    ...
    if (status === 429 && retryCount < 3) {
      ...
      await sleep(delay);
      return getOrderBuyerInfo(lwaToken, orderId, retryCount + 1);
    }

    // 403 = app SP-API sans rôle PII configuré côté Seller Central
    if (status === 403) {
      Logger.warn('amazon-connector', 'AMZ_BUYER_INFO_FORBIDDEN', {
        orderId,
        note: 'Permissions PII insuffisantes — vérifier rôle "Direct-to-Consumer Shipping" dans Seller Central',
      });
      return {};
    }
...
// Utilise un Restricted Data Token (RDT) pour accéder aux données PII
// =====================================================================
async function getOrderAddress(lwaToken, orderId, retryCount = 0) {
  try {
    await sleep(400);

    // ── Obtenir un RDT pour accéder aux PII Name/Phone ──────────────
    const rdt = await getRestrictedDataToken(lwaToken, orderId, ['shippingAddress']);
    const activeToken = rdt || lwaToken;

    if (!rdt) {
      Logger.warn('amazon-connector', 'AMZ_ADDRESS_NO_RDT', {
        orderId,
        note: 'RDT non disponible, appel avec LWA token — PII probablement masqués',
      });
    }
    ...
    if (status === 429 && retryCount < 3) {
      ...
      await sleep(delay);
      return getOrderAddress(lwaToken, orderId, retryCount + 1);
    }
